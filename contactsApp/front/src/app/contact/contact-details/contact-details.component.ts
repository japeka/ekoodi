...
export class ContactDetailsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if(id!==null) { //find the existing contact
      this.contactService2.findContactById(Number(id)).subscribe( (contact) => {
  ...
  onClickContactEvent(): void {
      ...
      if(this.contact.firstName && this.contact.lastName && this.contact.phone && this.contact.gender
        && this.contact.streetAddress && this.contact.city) {
          if(!this.isValidPhoneNumber(this.contact.phone)) {
            ...
            this.contactService2.addContact(this.contact)
              .subscribe((_contact) => {
                let cId = _contact.id;
    ...
    // update an existing contact //
      ...
      if(!this.isValidPhoneNumber(this.contact.phone)) {
        ...
        this.contactService2.updateContact(this.contact)
         .subscribe( (_contact) => {
            this.router.navigate(['/contacts']);
         },
         err => {
  ...
  onContactRemove(): void {
    this.contactService2.deleteContact(this.contact)
      .subscribe((_contact) => {
        this.router.navigate(['/contacts']);
      },
      err => {
